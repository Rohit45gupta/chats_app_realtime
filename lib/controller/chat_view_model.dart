...
class ChatViewModel with ChangeNotifier {
  ...
  getChatList({required String cid, required String otherId}) {
    ...
    starCountRef.orderByChild("dateTime").onValue.listen((DatabaseEvent event) {
      ...
      for (var element in data) {
        var dateTimeValue = element.child("dateTime").value;
  ...
  sendChat({required String otherUid}) async {
    ...
    var token = await getDeviceToken(otherUid);
    DatabaseReference starCountRef = FirebaseDatabase.instance.ref('messages/$chatId');

    DatabaseReference reference = FirebaseDatabase.instance.ref('user/$cid');
    var data = await reference.get();

    String? userName = "null";

    if (data.exists) {
      userName = data.child('name').value?.toString() ?? 'null';
    }
  ...
  Future<String?> getDeviceToken(String userId) async {
    DatabaseReference userRef =
        FirebaseDatabase.instance.ref('user').child(userId);
